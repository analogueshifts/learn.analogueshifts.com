...
export default function Landing() {
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    const allInformations = {
      ...
      previous:
        data?.background?.role === "Other"
          ? data?.background?.customRole
          : data?.background?.role,
      role: course?.name,
      background:
        data?.background?.coursesTaken + " " + data?.background?.listedCourses,
      experienceLevel: data?.background?.experienceLevel,
      availability: formData.commitment,
      goal: formData.goal,
      jobSupport: formData.jobSupport,
    ...
    for (const key in allInformations) {
      d.append(key, allInformations[key as keyof typeof allInformations]);
    }
    ...
    try {
      ...
      if (response.ok) {
        router.push(`/courses/${slug}/apply/success`);
      }
    } catch (error) {
      notifyUser(
        "error",
          ...
          >
                ...
                <div className="flex flex-col gap-7">
                  ...
                  {formData.goal === "Other" && (
                    <input
                      ...
                      onChange={(e) =>
                        handleChange("goalOther", e.target.value)
                      }
                    />
                  )}
                </div>
            ...
            <div className="w-full flex items-start gap-1.5">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex flex-col gap-7">
                  <RadioGroup
                    onValueChange={(v) => handleChange("mentorship", v)}
